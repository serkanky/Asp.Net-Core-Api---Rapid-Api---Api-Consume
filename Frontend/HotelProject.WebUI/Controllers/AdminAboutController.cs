...
namespace HotelProject.WebUI.Controllers
{
    public class AdminAboutController : Controller
    {
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:2450/api/About/{id}");
        ...
        public async Task<IActionResult> UpdateAbout(UpdateStaffViewModel model)
        {
            ...
            var responseMessage = await client.PutAsync("http://localhost:2450/api/About/", stringcontent);
            if (responseMessage.IsSuccessStatusCode)
            {
