...
namespace HotelProject.WebUI.Controllers
{
    public class ServiceController : Controller
    {
        ...
        public async Task<IActionResult> AddService(CreateServiceDto createServiceDto)
        {
            ...
            var responseMessage = await client.PostAsync("http://localhost:2450/api/Service", stringcontent);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.DeleteAsync($"http://localhost:2450/api/Service/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync($"http://localhost:2450/api/Service/{id}");
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateService(UpdateServiceDto updateServiceDto)
        {
            ...
            StringContent stringcontent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("http://localhost:2450/api/Service/", stringcontent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
